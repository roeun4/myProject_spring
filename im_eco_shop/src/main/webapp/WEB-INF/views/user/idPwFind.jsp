...
<div class="container shadow rounded p-5 mt-5 w-50">
  ...
  <div class="tab-content">
    <div id="menu1" class="container tab-pane ${find=='id'?'show active':''}"><br>
	    <div id="findIdSuccess" style="display: none;">
	    	<p>가입시 아이디는 <b><span id="findId"></span></b>입니다</p>
	    	<a href="<c:url value="login.do" />" class="btn btn-primary mt-3 w-100">로그인</a>
	    </div>
    	<div id="findIdBefore">
	    	<p>이름과 휴대폰 번호를 입력하세요.</p>
	    	<input class="form-control mb-3" type="text" id="name" name="name" placeholder="이름"/>
	    	<input class="form-control mb-3" type="text" id="tel" name="tel" placeholder="전화번호"/>
    ...
    <div id="menu2" class="container tab-pane fade ${find=='pw'?'show active':''}"><br>
    	<div id="findPwSuccess" style="display: none;">
	    	<p>
	    		<small>
	    			해당 이메일로 임시 비밀번호를 보냈습니다.<br>
	    			로그인 후 마이페이지에서 비밀번호를 변경하시길 바랍니다.
	    		</small>
	    	</p>
	    	<a href="<c:url value="login.do" />" class="btn btn-primary mt-3 w-100">로그인</a>
	    </div>
    	<div id="findPwBefore">
	    	...
	    	<input class="form-control mb-3" type="text" id="email" name="email" placeholder="이메일"/>
...
<script>
	function idFind() {
		...
		$.ajax({
			...
			type:"post",
			// 파라미터 data는 서버에서 전송한 id값
			success:function(data){
				console.log(data); // 빈공백 "" 전송됨
				// 자바스크립트에서 빈공백("")과 0은 false와 같다
				// null과 ""은 다르다
				// if(data == false){ 이 조건은 거짓
				if(data == false){ // data==0, data==""는 data==false와 같은 조건
