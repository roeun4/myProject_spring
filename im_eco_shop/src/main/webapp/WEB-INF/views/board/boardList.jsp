...
<div class="container w-100 mt-5">
    ...
    <div>
    	<form action="list.do" id="searchForm" method="post">
    		...
    		<input type="hidden" value="${pDto.cntPerPage}" name="cntPerPage"/>
    		
	    	<div class="d-flex justify-content-center">
	    		<select class="form-select form-select-sm me-2" style="width: 150px;" name="searchType">
	    		...
	    		<input type="text" id="keyword" name="keyword" placeholder="검색어 입력" style="width: 250px;" class="form-control rounded-0 rounded-start" value="${pDto.keyword}">
    ...
    <div class="d-flex justify-content-between mb-2">
    	...
    	<div>
    		<select class="form-select form-select-sm" id="cntPerPage">
    			...
    			<option value="15" ${pDto.cntPerPage == 15 ? 'selected':''}>게시글 15개</option>
    			<option value="20" ${pDto.cntPerPage == 20 ? 'selected':''}>게시글 20개</option>
    		</select>
    	</div>
    	</c:if>
    	
    	<!-- 검색이 있는 경우의 게시글 개수 선택 -->
    	<c:if test="${pDto.searchType != null && pDto.searchType != ''}">
    	<div>
    		<select class="form-select form-select-sm" id="cntPerPage">
    			<c:choose>
    				<c:when test="${pDto.totalCnt <= 10}">
                     <option value="10" ${pDto.cntPerPage == 10 ? 'selected':''}>선택없음</option>
    				</c:when>
    				<c:when test="${pDto.totalCnt > 10 && pDto.totalCnt < 15}">
    					<option value="10" ${pDto.cntPerPage == 10 ? 'selected':''}>게시글 10개</option>
		    			<option value="15" ${pDto.cntPerPage == 15 ? 'selected':''}>게시글 15개</option>
    				</c:when>
    				<c:when test="${pDto.totalCnt >= 15 && pDto.totalCnt < 20}">
		    			<option value="10" ${pDto.cntPerPage == 10 ? 'selected':''}>게시글 10개</option>
		    			<option value="15" ${pDto.cntPerPage == 15 ? 'selected':''}>게시글 15개</option>
    				</c:when>
    				<c:otherwise>
	...
	<%-- 	
		XSS(cross-site Scripting 공격 : 웹사이트에 스크립트 코트를 주입시켜서 공격하느 해킹 기법)
		JSP에서는 XSS 공격 차단하는 방법
		<c:out />를 사용하여 입력된 태그를 브라우저가 인식하지 못하도록 문자열로 변환시켜 방어함
	 --%>
	<tbody>
		...
		<tr>
			...
			<td><a href="<c:url value="view.do?bid=${dto.bid}&viewPage=${pDto.viewPage}&searchType=${pDto.searchType}&keyword=${pDto.keyword}&cntPerPage=${pDto.cntPerPage}"/>"><c:out value="${dto.subject}"/></a></td>
			<td>${dto.hit}</td>
			<td>${dto.reg_date}</td>
		</tr>
		</c:forEach>
...
/* 	$("#cntPerPage").change(function() {}) */
	$("#cntPerPage").change(()=>{
		...
		/* alert(cntVal); */
		/* location.href="<c:url value='list.do?viewPage=${pDto.viewPage}&searchType=${pDto.searchType}&keyword=${pDto.keyword}'/>"+cntVal; */
		// 이벤트가 발생하면 항상 1페이지로 이동하도록 viewPage=1 로 설정해줌
